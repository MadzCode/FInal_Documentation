...
- At Death we part is a 2.5D Platformer that utilises light and dark mechanics to help the player progress through the level. The character will have two states; A light state and a dark state, this allows the player to pass through the shadow areas whilst in the dark state and the light areas whilst in the dark state. This then allows the player to stand on platforms of the opposite state (Light character can stand on the dark areas and vise versa).


- My initial goal is to create a functional swapping mechanic for the player that I can then add to by adding powers to the different states and the making puzzles around that. Then after that is completed I will then add more mechanics and polish to the game.    


- The most immediate challenge I may face is making the light mechanic work and only light up the part of the platform it is touching. Another issue may be miscommunication from team members.  
...
- Since I am working on a group project I feel the best methodology would be Agile methodology.
...
- The game's artistic inspiration relies heavily on Paper Mario (Nintendo of Europe SE, 2004). The 2.5D style was very visually appealing with the 2D sprites on a 3D world. After discussing with my group it became the main characters and other NPCs main aesthetic.


- All of these characters have quirky personalities which makes them enjoyable to listen from and learn about.


- The fun mini games also make this game truly unforgettable as you can be playing bingo one moment and fighting dragons the next.
...
- Little Nightmares excelled at adding suspense to always keep the player on edge. whether that would be falling platforms or hiding from a mutated blind man. Its way of storytelling intrigued me and I hope to use that immersive storytelling inside my game as well.
...
Ori is a 2.5 Platformer game where the main character 'Ori' traverses through the world gaining power ups as he goes. I found the 2.5D artstyle to look very interesting with this almost storybook looking artstyle.
...
- "Ori and the Blind Forest's gameplay combines platformer elements with open-world exploration. Players control Ori who can jump, run on walls and use various abilities to overcome obstacles and fight enemies."
...
- Inside:
Inside is a 2.5D game where the player must travel through a bleak empty world using the environment to put together the story and figure out the character's objective.
...
- This tutorial is to help brush up my knowledge of splines to create a smooth transition on the side.
...
- After talking to the designer I was told he wanted a static camera instead of a moving spline. This tutorial teaches me how to create a resident evil style camera mechanic which I feel could be very visually appealing.
...
- I didn't like how the camera moved so I disabled the movement and made the camera completely static.
...
# 2D Movement & Animation
 ...
 - I decided to remake the old third person character template with a 2D one created by the artist of the group. This new 2D character is a better fit for the 2.5D aesthetic that we are going for in this game.
...
- To make the lighting more realistic I added cast shadow in the details tab, this will make the lighting look more realistic on the character.
- To ensure that this works in the materials I added a MaskedLitSpriteMaterial so that the light works on the character correctly.
...
- I was looking into creating a 2D flashlight for the game but I was unsure how to implement it so I looked at copilot to help me learn about how to create it.
...
- After tweaking the flashlight I managed to make it work correctly when the player turned left.
...
## Cursor mechanic
- To make the game feel more stylised I added a custom cursor to the game, this step was easy but I am eventually going to make the cursor change when clicked to be more visual.
...
- After that the basics of the shadow platform was done and all that needed to be done was to add a timer onto the shadow platform to avoid being able to use the platforms to walk over obstacles.


- A common complaint whilst game testing was that you had infinite platforms so they would just be able to walk over the obstacles to get to the end of the game.
...
- I was also tasked with creating a light dash mechanic to give the player more manoeuvrability and to allow the light state character to have a state of its own.
...
- To create more of an enjoyable platforming experience I created some moving spline platforms to allow for more interesting levels.


- There originally was an issue with the vertical platforms where the platforms would spin at the top of the spline, however with my designer we were able to figure out the issue and quickly fix it.
...
- The credits are triggered by a collision box behind the door which will play the credits then take you to the main menu.
...
- To add functionality to the game I created a main manu and pause screen to allow them to change settings, pause, or quit the game.
...
- To give the game an objective I created a door and key system, the player must collect 3 keys in order to pass through the door.
- I thank my fellow student Evie Green for assisting me in making the 3 key mechanics work.
...
- To create the script I used a CSV file then import it into Unreal to use as the dialogue sheet.
...
## Light System
- Due to time constraints and being repeatedly let down by my designer I had to remake the entire game idea from scratch whilst trying to keep the overall theme of loss and acceptance intact. Instead of having 5 levels there will now be one giant level with 3 sections (attic, bedroom, basement) and the movement controls and mechanics have been simplified indefinitely or when we meet schedule deadlines. This change was unfortunate as I wanted to trust the designers reassurance that he would get everything done on time, however, they never delivered. I have also diminished the designers rights and I am now acting as lead designer and I have stationed the old designer to doing background assets and sound design.
...
- Originally the game was supposed to have 5 levels each representing a different stage of grief which had to unfortunately be scrapped, as well as the old dialogue system as it did not make any sense anymore. Then the game got cut down to 3 levels but the third level didn't really get worked on. It was then cut down to one level but again it was never really worked on so it had to be scrapped. It is now down to one level with all of the story gone besides some subtle hints.


## Flashlight System


- Unfortunately the flashlight system had to be completely removed from the game as it was useless. It was originally going to be used on ghosts and turn them into normal platforms when the light was shining on it but due to time constraints it had to be scrapped.
...
- I mainly let the playtesters blind test as I didn't want to hold their hands through the whole game. I noticed a lot of the struggled with the first set of jumps so I tried to make them easier by adjusting.
- Because of the jumps many testers thought it was a bit hard to pass though the section.
...
https://github.com/GameDevFinal/At_Death_We_Part
...
- I enjoyed managing my group and giving them different assignments to help improve the game, from asking my artist to create different assets that will help improve the games overall storytelling, to asking my designer to create music using the different themes and instruments I chose to make the music more immersive and emotional.
...
- One thing I struggled with last time was documentation as I would just put everything I did into the notes making it unorganised and hard to find what I am looking for immediately, costing me time that could be spent on working on the project.
...
- “Microsoft Copilot: Your AI Companion.” n.d. Microsoft Copilot: Your AI Companion. Accessed May 23, 2025. https://copilot.microsoft.com/chats/oJ65TiA8mrC8hjLXF4Bjt.
...
- “Paperpile.” n.d. Accessed May 23, 2025. https://app.paperpile.com/my-library/all.
...
- “ZoteroBib: Fast, Free Bibliography Generator - MLA, APA, Chicago, Harvard Citations.” n.d. Accessed May 23, 2025. https://zbib.org/.
...
- James Davies, STYLISE-IN_GAME_ASSETS, 2025
...
- Marcus Nobs, IN-GAME-MUSIC, 2025